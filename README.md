...
> - **`V4-china` 分支（v4 新架构，基本可用）**：搜索、活动、签到、Read-to-Earn、微信推送（PushPlus / ClawBot）等核心功能已实测通过（2026-08），**可以切换日常使用**；新架构仍在迭代，偶发小问题属正常，追求稳定可继续留在 `main` 分支。
>
> 两分支配置文件结构不同，**互不兼容**；迁移 `V4-china` 请按其 README 重新配置 `.env` / `config.json`。
